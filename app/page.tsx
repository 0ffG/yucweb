      ...
      <main className="w-full">
        <section className="relative h-[850px] overflow-hidden">
          ...
          <div className="relative text-center text-white flex flex-col items-center justify-center h-full px-4">
            ...
            <Link href="/donate" className="mt-12 px-10 py-4 rounded-full bg-gray-900 text-white font-medium text-lg transition-all hover:shadow-xl hover:bg-gray-800 hover:scale-105">BAĞIŞ YAP</Link>
            <Link href="/schools" className="mt-4 px-8 py-3 rounded-full bg-blue-600 text-white font-semibold text-md transition-all hover:shadow-md hover:bg-blue-500 hover:scale-105">Okul Profillerini Görüntüle</Link>
          </div>
        </section>
        ...
        <section className="bg-white py-16 px-6">
          <div className="max-w-5xl mx-auto space-y-12">
            {news.map((item) => (
              <div key={item.id} className="flex flex-col md:flex-row items-center justify-between gap-8 pb-8 border-b border-gray-200 last:border-none">
                <div className="md:w-1/2 text-center md:text-left">
                  <h3 className="text-2xl font-semibold text-gray-800 mb-2">{item.title}</h3>
                </div>
                {item.image && (
                  <div className="md:w-1/2">
                    <img src={item.image} alt={item.title} className="w-full h-64 object-cover rounded-xl shadow-md" />
                  </div>
                )}
              </div>
            ))}
